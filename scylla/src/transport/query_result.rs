...
impl QueryResult {
    ...
    ///
    /// ```
    pub fn into_rows_response(self) -> Result<Option<RowsQueryResponse>, RowsParseError> {
        self.raw_metadata_and_rows
            .map(|raw_rows| {
                let raw_rows_with_metadata = raw_rows.deserialize_metadata()?;
                Ok(RowsQueryResponse {
    ...
    fn test_query_result() {
        ...
        // Check col specs
        {
            // Not RESULT::Rows response -> no column specs
            {
                ...
                let qr = rqr.into_rows_response().unwrap();
            ...
            // RESULT::Rows response -> some column specs
            {
                ...
                let qr = rqr.into_rows_response().unwrap().unwrap();
        ...
        // All errors are checked.
        {
            ...
            // RESULT::Rows with 1 row
            {
                ...
                let qr_good_data = rqr_good_data.into_rows_response().unwrap().unwrap();
                let qr_bad_data = rqr_bad_data.into_rows_response().unwrap().unwrap();
